...
# This is the MOST IMPORTANT rule for TTS quality:
# - ALWAYS write Hindi in DEVANAGARI: नमस्ते, आपका बैलेंस, सोने का भाव
# - NEVER write romanized Hindi: "namaste", "aapka balance", "sone ka bhav"
# - Even single Hindi words MUST be in Devanagari: जी, अच्छा, बिल्कुल, धन्यवाद
# - Romanized Hindi sounds TERRIBLE in Murf TTS — it reads it as English
# - If user speaks ANY Hindi/Hinglish, respond ENTIRELY in Devanagari script
# - This applies to EVERY response, EVERY tool result explanation, EVERYTHING
# VIOLATION OF THIS RULE = BROKEN TTS = FAILED CONVERSATION
...
# ESCALATION PROTOCOL — Day 7: Know When to Ask for Human Help
You have escalation tools. When certain situations arise, you MUST escalate to
a human team member instead of trying to solve the problem yourself.

## TWO ESCALATION TRIGGERS (and ONLY two):

### 1. FRAUD DETECTION — escalation_type: "fraud"
...
- "Money is missing/gone from my account"
- Hindi: "किसी ने पैसा निकाल लिया", "पैसा गायब", "अकाउंट हैक"

### 2. REGULATORY/COMPLIANCE — escalation_type: "regulatory"
Trigger when user says:
- "I want to file a formal complaint"
- "I'll take legal action"
- "RBI ombudsman"
- "Banking codes violation"
- "Consumer forum/consumer court"
- "Compliance issue"
- Hindi: "शिकायत दर्ज करना है", "कानूनी कार्रवाई", "उपभोक्ता फोरम"

## WHAT IS NOT AN ESCALATION (DO NOT trigger on these):
- Normal balance/transaction queries
- General banking questions
- User asking for help (answer it yourself)
- User frustrated but the issue is within your scope (help them)
- "I have a problem" without fraud/regulatory language
- User unhappy with rates/fees (explain, don't escalate)
- Questions about UPI, schemes, loans, gold prices, FD
DO NOT escalate normal conversations. You are capable — handle them.

## MANDATORY PERMISSION PROTOCOL (before calling create_escalation_ticket):
When you detect a fraud or regulatory trigger:
1. ACKNOWLEDGE their concern empathetically FIRST
   Example: "I understand this is very concerning."
2. EXPLAIN what you want to do:
   "I want to escalate this to our human team for priority handling."
3. INFORM what info you will share:
   "I will send a brief summary of your issue — NOT your full conversation,
   just the key details. No account numbers, no personal IDs."
4. ASK explicit permission:
   "May I create a priority ticket for you?"
5. WAIT for their YES before calling the tool.
6. IF USER SAYS NO: respect it immediately. Say:
   "I will not share anything without your permission.
   Here are some alternatives: Cyber Crime helpline 155260 or 1930,
   RBI helpline 1800-222-490, or your bank number on the back of your card.
   You can always call back if you change your mind."

## AFTER ESCALATION:
- You will get a reference ID like VP-2026-10001
- SPEAK IT CLEARLY AND SLOWLY
- Offer to repeat: "Would you like me to say that again?"
- Set expectations:
  - Critical urgency: "Our team will review within 1 hour"
  - High urgency: "Our team will review within 4 hours"
  - Medium urgency: "Our team will review within 24 hours"
- Say: "They will call you back with an update."

## PII RULES FOR SUMMARIES:
NEVER include in issue_summary: account numbers, Aadhaar, PAN, phone numbers,
OTP, PIN, CVV, passwords, card numbers. Write generically:
GOOD: "User reports unauthorized withdrawal from their savings account"
BAD: "User reports 5000 rupees withdrawn from account 1234567890"

## CHECKING EXISTING TICKETS:
If user says "check my ticket", "status of VP-...", "meri complaint ka kya hua",
or mentions a VP-XXXX reference, call check_escalation_status.

# FINAL REMINDER — HINDI SCRIPT (MOST IMPORTANT RULE FOR TTS):
# NEVER use romanized Hindi. ALWAYS use DEVANAGARI script for Hindi.
# "namaste" is WRONG → "नमस्ते" is CORRECT
# "aapka balance" is WRONG → "आपका बैलेंस" is CORRECT
# "sone ka bhav" is WRONG → "सोने का भाव" is CORRECT
# If user speaks Hindi, your ENTIRE response must be in DEVANAGARI.
# Romanized Hindi sounds TERRIBLE in TTS. This is NON-NEGOTIABLE."""
...
class VoicePayAgent(Agent):
    ...
    def __init__(self, stats: SessionStats, persona: dict[str, Any] | None = None, user_memory: dict[str, Any] | None = None) -> None:
        ...
        self._conv_logger: ConversationLogger | None = None
        self._escalation_context: dict[str, Any] | None = None  # detected trigger awaiting consent
    ...
    async def create_escalation_ticket(
        ...
        """Create a human-help escalation ticket. Call this ONLY AFTER you have:
        1. Detected a fraud or regulatory trigger in the conversation
        2. Explained to the user what you will share (a brief summary, no PII)
        3. Asked the user for EXPLICIT permission and they said YES

        Args:
            issue_summary: Brief 2-3 sentence description of the issue.
                Do NOT include account numbers, Aadhaar, PAN, phone numbers,
                OTP, PIN, CVV, or passwords. Write generically.
                Example: "User reports unauthorized withdrawal from savings account.
                Transaction happened today morning. User did not share OTP with anyone."
            escalation_type: Either 'fraud' or 'regulatory'. Use 'fraud' for
                unauthorized transactions, hacking, stolen cards. Use 'regulatory'
                for formal complaints, legal threats, ombudsman references.
            urgency: One of 'critical', 'high', 'medium', 'low'.
                - critical: active ongoing fraud happening RIGHT NOW
                - high: fraud with money already lost, or legal threats
                - medium: standard regulatory complaint
                - low: general process inquiry about complaints

        Returns a reference ID (VP-2026-XXXXX) that you MUST speak clearly to the user.
        """
        self.stats.bump_tool("create_escalation_ticket")
        self.stats.escalations += 1

        if not self._user_id:
            return "ERROR: Cannot create escalation without user identity. Ask the user to identify themselves first."

        if escalation_type not in ("fraud", "regulatory"):
            escalation_type = "fraud"  # safe default
        ...
        # Duplicate detection — don't create two tickets for same issue
        existing = await find_duplicate_escalation(pool, self._user_id, escalation_type)
        if existing:
            ...
            await append_to_escalation(
                ...
                new_room_name=self.stats.room,
            )
            if self._conv_logger:
                await self._conv_logger.log_system_event(
            ...
            return (
                f"EXISTING TICKET UPDATED: Reference {ref} (status: {existing['status']}). "
                f"Tell the user: 'You already have an open ticket — {ref}. "
                f"I have added today's conversation details to it. "
                f"Our team is actively working on it. "
                f"Is there anything else I can help you with?'"
            )

        # Determine what the agent already checked
        tools_used = list(self.stats.tool_calls.keys())
        what_checked = f"Agent checked: {', '.join(tools_used)}" if tools_used else "Agent had not yet checked any tools before escalation."

        # Get caller name from memory
        caller_name = None
        if self._user_memory and self._user_memory.get("name"):
            caller_name = self._user_memory["name"]

        # Get trigger phrases from detection context
        trigger_phrases = []
        if self._escalation_context:
            trigger_phrases = self._escalation_context.get("trigger_phrases", [])
        ...
        esc = await create_escalation(
            ...
            metadata={"persona": self.persona["name_display"]},
        )

        ref_id = esc["reference_id"]

        # Copy conversation transcript to the ticket
        await copy_transcript_to_escalation(
            pool, escalation_id=str(esc["id"]), room_name=self.stats.room,
        )

        if self._conv_logger:
            await self._conv_logger.log_system_event(f"Escalation created: {ref_id}")
        ...
        # Build response time expectation
        time_map = {
            "critical": "1 hour",
            "high": "4 hours",
            "medium": "24 hours",
        ...
        return (
            ...
            f"Tell the user clearly: "
            f"'I have created a priority ticket for you. "
            f"Your reference number is {ref_id}. Please note this down. "
            f"I will repeat it: {ref_id}. "
            f"Our team will review this within {response_time} and call you back with an update. "
            f"Is there anything else I can help you with right now?' "
            f"Speak the reference ID SLOWLY and CLEARLY. Offer to repeat it."
        )
    ...
    async def check_escalation_status(
        ...
        """Check the status of an existing escalation ticket by reference ID.

        Args:
            reference_id: The VP-YYYY-XXXXX reference number given to the user.
                Example: VP-2026-10001. User might say it as "VP 2026 10001" or
                "my ticket number is 10001" — normalize to VP-YYYY-XXXXX format.

        Call when user says "check my ticket", "what happened to my complaint",
        "VP-2026-XXXXX status", or asks about a previous report/complaint.
        """
        self.stats.bump_tool("check_escalation_status")
        ...
        if not esc:
            return (
                f"NOT FOUND: No ticket found with reference '{reference_id}'. "
                f"Tell the user: 'I could not find a ticket with that reference number. "
                f"The format is VP-YYYY-XXXXX, for example VP-2026-10001. "
                f"Could you double-check the number? Or would you like me to help with something else?'"
            )
        ...
        status_messages = {
            "open": "Your ticket is open and waiting for review by our team.",
            "in_progress": "A team member is actively working on your case.",
            "awaiting_callback": "We have an update and will be calling you back shortly.",
        ...
        await self._push_visual("escalation_status", {
        ...
        return (
            f"FOUND: Ticket {esc['reference_id']} — "
            f"Type: {esc_type}, Status: {status}, Urgency: {urgency}, "
            f"Created: {created}. "
            f"Tell the user naturally: '{status_msg}' "
...
async def voicepay_session(ctx: JobContext) -> None:
    ...
    def _on_user_input(ev: Any) -> None:
        if getattr(ev, "is_final", False):
            ...
            # Day 7: Log user message to conversation_logs (fire-and-forget)
            if conv_logger:
                asyncio.ensure_future(
                    conv_logger.log_user_message(transcript, language=stats.language_detected or "en")
                )

            # Day 7: Escalation trigger detection (non-blocking)
            trigger = detect_escalation_trigger(transcript)
            if trigger:
                logger.info("ESCALATION TRIGGER DETECTED: type=%s phrases=%s", trigger["type"], trigger["trigger_phrases"])
                # Store context — the system prompt instructs the agent to ask permission
                if hasattr(session, 'current_agent') and session.current_agent:
                    session.current_agent._escalation_context = trigger
    ...
    async def _log_session_summary() -> None:
        ...
        # ---------------------------------------------------------------
        # Day 8: ALWAYS persist call analytics (no consent needed — no PII)
        # ---------------------------------------------------------------
        try:
            pool = await get_pool()
            tool_names = set(summary["tool_calls"].keys()) if summary["tool_calls"] else set()

            # Determine success criteria
            success_criteria: list[str] = []
            ELIGIBILITY_TOOLS = {"scheme_eligibility", "loan_eligibility"}
            RATE_TOOLS = {"gold_silver_prices", "rbi_rates", "fd_rate_comparison"}

            if tool_names & ELIGIBILITY_TOOLS:
                success_criteria.append("eligibility_check")
            if "document_checklist" in tool_names:
                success_criteria.append("document_list")
            if tool_names & RATE_TOOLS:
            ...
            if stats.security_blocks > 0:
                success_criteria.append("scam_education")

            # Classify outcome
            if summary["user_turns"] == 0:
                outcome = "abandoned"
                outcome_reason = "User disconnected before interaction"
            elif summary["tool_errors"] > 0 and not success_criteria:
                outcome = "error"
                outcome_reason = f"Tool errors: {summary['tool_errors']}"
            elif success_criteria:
                outcome = "success"
                outcome_reason = f"Criteria: {', '.join(success_criteria)}"
            else:
                outcome = "failed"
                outcome_reason = "No success criteria met"

            # Build latency breakdown
            eou_vals = [s.eou_delay_ms for s in stats.latencies if s.eou_delay_ms]
            llm_vals = [s.llm_ttft_ms for s in stats.latencies if s.llm_ttft_ms]
            tts_vals = [s.tts_ttfb_ms for s in stats.latencies if s.tts_ttfb_ms]
            latency_breakdown = _json.dumps({
                ...
                "samples_count": len(stats.latencies),
            })
            ...
            async with pool.acquire() as conn:
                await conn.execute(
                    ...
                    call_channel,
                    stats.started_at,
                    int(summary["duration_s"]),
                    summary["user_turns"],
                    summary["agent_turns"],
                    _json.dumps(summary["tool_calls"] or {}),
                    list(tool_names),
                    summary["tool_errors"],
                    stats.security_blocks,
                    stats.escalations,
                    stats.language_detected or "en",
                    outcome,
                    outcome_reason,
                    success_criteria,
                    summary["avg_latency_ms"],
            ...
            logger.info("CALL ANALYTICS persisted: room=%s outcome=%s", summary["room"], outcome)
        except Exception as e:
            logger.warning("call_analytics persist failed (non-fatal): %s", e)
    ...
    try:
        ...
        # Day 7: Conversation Logger — logs EVERY message for full audit trail
        conv_logger = ConversationLogger(
            room_name=ctx.room.name,
            user_id=user_id,
            persona=persona["name_display"],
        )
        agent._conv_logger = conv_logger
